"""Work with tmux sessions, windows, and panes from Python, plus named background sessions keyed by sid. Use this when code needs to read what is on a terminal, send input to a TUI or other rich terminal app, build pane layouts, search text across terminals, or keep a terminal open that you and the user can both see. For plain line-oriented programs (REPLs, shells, ssh) use `ptymini.skill`'s bgterm sessions instead. They share this module's wait parameters (`wait_ms`, `until=`, `settle_ms`). tmux is for when the terminal itself matters.

fastmux runs the `tmux` binary. Each handle identifies its object by tmux's server-unique id (`$1`, `@1`, `%1`). Renaming sessions or renumbering windows does not invalidate handles. A handle is a dict of the fields tmux reports (`p.width`, `p.cmd`, `p.dead`). `refresh()` updates it in place. A failed tmux command raises `TmuxError` with tmux's own message.
...
- `start_session(sid=None, cmd=None, ...)` creates or reuses the named session. Created sessions are detached and managed, with `remain` set and the dead-pane banner cleared. The created pane remains the target for `sid` after splits. With `sid=None`, it generates a `fastmux-`-prefixed name.
- `managed_sessions()` lists sessions created by `start_session`.
- `close(sid)` kills the session owning the specified pane. Passing a `%pane_id` also kills its owning session.
...
Writing to a pane: `p.send(text)` pastes literal text through a tmux buffer with no key-name interpretation, `p.send_keys('C-c', 'Enter')` sends tmux key names, and `p.interrupt()` is Ctrl-C. Each polls afterwards and returns a fresh `Capture`. Polling waits for the pane to differ from the last capture returned for it. Output that arrived between calls satisfies the next `poll` immediately. Output takes time to appear, so pass `wait_ms` rather than re-reading immediately, e.g. `p.send('ls\n', wait_ms=2000)`. `p.wait(timeout_ms=...)` blocks until the pane's command exits and returns its status (`None` on timeout); the pane needs `remain` to outlive its command.

Waiting: every wait is bounded and returns the latest `Capture` on timeout rather than raising, so the caller reads what came back and decides. Keep waits short: a few seconds of `wait_ms` and 1-3 seconds of `settle_ms`. Read each capture before deciding whether to poll again. If a spinner is still animating after three short polls, reconsider what you are waiting for.

`until=` waits for a regex to match the capture and returns within one `interval_ms` of matching. It checks text already present, too. Choose a pattern that only the awaited output will produce.

With `settle_ms`, polling continues after the wait until the capture has stayed unchanged for that duration. This takes the full `settle_ms` even when the pane was already quiet. Each change restarts the quiet timer. The settle phase can use any time remaining in `wait_ms` and at most `settle_ms` beyond that deadline.

A spinner or clock prevents the pane from settling. Use `until` to wait for identifiable output instead.

`poll(sid, settle_ms=2000)` watches without sending input.
...
Important:
Use only ids, `target` strings, and sids obtained from fastmux objects or agreed with the user. Hand-built name targets can be ambiguous to tmux. For example, a session named `5` can resolve as a window index.

The tmux server is shared. Killing, resizing, or writing to another session's panes changes what an attached user sees. Work in sessions you created and kill them when done.

`sid=None` addresses the user's current pane. Use it only when they offer it.

tmux snapshot polling is not a byte-perfect log transport: for huge structured outputs, write to a file instead. Run `doc(fastmux.core)` or `doc(fastmux.bg)` for module overviews and `doc(obj)` for full parameter docments before first use.
"""
