...
# %% auto #0
__all__ = ['SEARCH_LINES', 'TmuxError', 'Session', 'new_session', 'Pane', 'Capture', 'Screen', 'Panes', 'Window', 'Windows',
           'SearchMatch', 'SearchResults', 'Sessions', 'tmux', 'current_pane']
...
_pane_f = dict(id='pane_id', session='session_name', window_id='window_id', win='window_index', idx='pane_index',
    ...
    pid='pane_pid', hist='history_size', cx='cursor_x', cy='cursor_y', dead_status='pane_dead_status',
    in_mode='pane_in_mode', scroll_pos='scroll_position', title='pane_title')
_ints  = {'win','idx','pid','width','height','hist','cx','cy','n_wins','n_panes'}
_bools = {'active','dead','attached','in_mode'}
_optint = {'scroll_pos'}  # blank outside copy-mode
...
def _parse(fields, line):
    ...
    for k in fields.keys() & _optint: d[k] = int(d[k]) if d[k] else None
    return d
...
class Pane(AttrDict):
    ...
    def refresh(self):
        ...
        return self
    def fmt(self, f):
        "tmux's answer for any `#{format}` string, evaluated on this pane"
        return _tmux('display-message','-p','-t',self.id,f)
    @property
...
# %% ../nbs/00_core.ipynb #e6c57a66
_SGR_KEEP = {0,1,2,3,4,7,21,22,23,24,27}

def _quiet(text):
    "SGR with color params dropped, keeping text attributes (bold/dim/italic/underline/reverse and their resets)"
    def f(m):
        codes, kept, i = m.group(1).split(';'), [], 0
        while i < len(codes):
            c = int(codes[i] or 0)
            if c in (38, 48): i += 2 if codes[i+1:i+2] == ['5'] else 4  # extended color: skip its params too
            elif c in _SGR_KEEP: kept.append(codes[i] or '0')
            i += 1
        return '\x1b[' + ';'.join(kept) + 'm' if kept else ''
    return re.sub(r'\x1b\[([0-9;]*)m', f, text)

def _mark(line, x):
    "Overlay the cursor cell at column `x` with ▮, skipping SGR sequences when counting"
    res, col, done, i = [], 0, False, 0
    while i < len(line):
        m = re.match(r'\x1b\[[0-9;]*m', line[i:])
        if m: res.append(m.group()); i += m.end(); continue
        res.append('▮' if col == x else line[i])
        done, col, i = done or col == x, col+1, i+1
    if not done: res.append(' '*(x-col) + '▮')
    return ''.join(res)

class Screen(AttrDict):
    "The visible screen plus geometry, cursor position, and source pane"
    def __repr__(self):
        status = '' if self.running else (f' · exited {self.exit_code}' if self.exit_code is not None else ' · dead')
        src = f'── {self.target} {self.id} · screen {self.width}x{self.height} · cursor {self.cx},{self.cy}{status}'
        lines = self.text.splitlines()
        while len(lines) <= self.cy: lines.append('')
        lines[self.cy] = _mark(lines[self.cy], self.cx)
        return '\n'.join([*lines, src])
    def _repr_pretty_(self, p, cycle): p.text(repr(self))

@patch
def screen(self:Pane, styles=False):
    "A `Screen` of the visible viewport (rows below the cursor included), recording the pane's last-seen state"
    self.refresh()
    text = _tmux('capture-pane','-p','-t',self.id).rstrip('\n')
    _seen[self.id, 'screen'] = (text, self.running, self.exit_code)
    if styles: text = _quiet(_tmux('capture-pane','-p','-e','-t',self.id).rstrip('\n'))
    return Screen(text=text, lines=tuple(text.splitlines()), width=self.width, height=self.height, cx=self.cx,
                  cy=self.cy, target=self.target, id=self.id, running=self.running, exit_code=self.exit_code)

# %% ../nbs/00_core.ipynb #0e1ce944
def _snapshot(p, lines, screen=False):
    if screen:
        c = p.screen()
        return c.text, c.running, c.exit_code
    c = p.display(lines)
    return c.text, c.n, c.running, c.exit_code

@patch
def poll(self:Pane, wait_ms=0, interval_ms=50, lines=80, screen=False, styles=False, until=None, settle_ms=0):
    """Wait up to `wait_ms` for the pane to differ from its last-seen state (returning at once if it
    ...
    if until is not None:
        ...
        base = _seen.get((self.id, 'screen' if screen else lines)) or _snapshot(self, lines, screen)
        while _snapshot(self, lines, screen) == base and time.monotonic() < deadline:
            time.sleep(min(interval_ms/1000, max(0, deadline-time.monotonic())))
    if settle_ms:
        last, quiet = _snapshot(self, lines, screen), time.monotonic()
        while time.monotonic()-quiet < settle_ms/1000 and time.monotonic() < deadline + settle_ms/1000:
            time.sleep(interval_ms/1000)
            cur = _snapshot(self, lines, screen)
            if cur != last: last, quiet = cur, time.monotonic()
...
def send(self:Pane, chars='', wait_ms=0, interval_ms=50, lines=80, screen=False, styles=False, until=None, settle_ms=0):
    ...
    if chars:
        ...
        try: _tmux('paste-buffer','-d','-b',buf,'-t',self.id)
            ...
            raise
    return self.poll(wait_ms, interval_ms, lines, screen, styles, until, settle_ms)

@patch
def send_keys(self:Pane, *keys, wait_ms=0, interval_ms=50, lines=80, screen=False, styles=False, until=None, settle_ms=0):
    "Send tmux key names to this pane, then `poll`"
    if keys: _tmux('send-keys','-t',self.id,*keys)
    return self.poll(wait_ms, interval_ms, lines, screen, styles, until, settle_ms)

@patch
def interrupt(self:Pane, wait_ms=0, interval_ms=50, lines=80, screen=False, styles=False, until=None, settle_ms=0):
    "Send `Ctrl-C` to this pane, then `poll`"
    return self.send_keys('C-c', wait_ms=wait_ms, interval_ms=interval_ms, lines=lines, screen=screen, styles=styles,
...
# %% ../nbs/00_core.ipynb #c218ec6a
@patch
def click(self:Pane, target, btn=0, **poll_kw):
    """Send an SGR mouse click (press+release) to this pane's application, then `poll` with `poll_kw`.
    `target` is a 1-based `(col,row)`, or a str (literal) or compiled regex located on the current
    screen -- the click lands on the bottom-most match's first cell (screens grow downward: the newest occurrence is the live one). Raises `ValueError` if it is not on screen."""
    if isinstance(target, tuple): col, row = target
    else:
        rx = target if isinstance(target, re.Pattern) else re.compile(re.escape(target))
        for row, line in reversed(list(enumerate(self.screen().lines, 1))):  # bottom-up: the newest occurrence is the live one
            m = rx.search(line)
            if m:
                col = m.start() + 1
                break
        else: raise ValueError(f'not on screen: {target!r}')
    return self.send(f'\x1b[<{btn};{col};{row}M\x1b[<{btn};{col};{row}m', **poll_kw)

@patch
def wheel(self:Pane, n=3, at=(1, 1), **poll_kw):
    "Send `n` SGR wheel events (up for positive `n`, down for negative), then `poll` with `poll_kw`"
    col, row = at
    return self.send(f'\x1b[<{64 if n > 0 else 65};{col};{row}M' * abs(n), **poll_kw)
