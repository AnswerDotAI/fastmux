...
polls for the response, `p.send_keys('C-c')` sends tmux key names,
`p.click('[menu]')` mouse-clicks a coordinate or on-screen text,
`p.wheel()` scrolls, and `p.wait()` returns an exit status. Waits can be
declarative: `until='READY'` blocks until the screen matches a regex,
`settle_ms=300` until it stops changing. See the [full
